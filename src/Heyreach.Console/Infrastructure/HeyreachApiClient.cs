...
public sealed class HeyreachApiClient : IDisposable
{
    private const string BaseUrl = "https://api.heyreach.io/api/public/";
    ...
    public HeyreachApiClient(string apiKey, bool verbose = false)
    {
        _verbose = verbose;
    ...
    public async Task<JsonElement> DeleteAsync(string path)
    {
        var p = NormalizePath(path);
        LogRequest("DELETE", p);
        var response = await _http.DeleteAsync(p);
        return await HandleResponseAsync(response);
    }
    ...
    private async Task<JsonElement> HandleResponseAsync(HttpResponseMessage response)
    {
        ...
        if (response.StatusCode == HttpStatusCode.TooManyRequests)
        {
            ...
            throw new HeyreachApiException($"Rate limited (429). Retry after {retryAfter:F0}s", 2);
        }

        if (!response.IsSuccessStatusCode)
        {
            var message = TryExtractErrorMessage(body) ?? $"API error: {response.StatusCode}";
            throw new HeyreachApiException(message, response.StatusCode >= HttpStatusCode.InternalServerError ? 2 : 1);
        }
